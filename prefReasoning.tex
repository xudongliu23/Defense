...
\frameT{Conclusion}{
	\begin{enumerate}
		\item When votes are total orders of candidates, computing a winner
					for a positional voting rule is computationally easy; not necessarily so,
					when they are LP-trees over combinatorial domains.
		\item For the cases when determining a winner is computationally hard,
					we solved this problem using ASP and empirically
					showed that ASP tools are \tit{effective} on large 
					instances (up to 3000 votes over up to 20 binary attributes).
	\end{enumerate}
}
